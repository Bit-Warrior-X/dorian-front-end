  ...
  <div class="waf-section-card">
    ...
    <div class="waf-table-wrapper">
      <table class="waf-table">
        ...
        <tbody>
          <tr v-for="rule in intervalRules" :key="rule.id">
            <td class="checkbox-col">
              <input
                ...
                aria-label="Select interval freq limit rule"
              />
            </td>
            <td>{{ rule.url }}</td>
            <td>{{ rule.time }}</td>
          ...
          <tr v-if="!intervalRules.length">
            <td colspan="7" class="empty-cell">No interval freq limit rules yet.</td>
          </tr>
        </tbody>
      </table>
    </div>
    <div v-if="isDialogOpen" class="dialog-overlay" @click.self="closeDialog">
      <div class="dialog-card" role="dialog" aria-modal="true" aria-label="Add interval freq limit rule">
        <div class="dialog-header">
          <h4>Add Interval Freq Limit Rule</h4>
    ...
    <div v-if="deleteTarget" class="dialog-overlay" @click.self="closeDeleteDialog">
      <div class="dialog-card dialog-confirm" role="dialog" aria-modal="true" aria-label="Delete interval rule">
        <div class="dialog-header">
          <h4>Delete Rule</h4>
        ...
        <div class="dialog-body">
          <p class="confirm-text">
            Are you sure you want to delete this interval freq limit rule?
          </p>
        </div>
        <div class="dialog-footer">
    ...
    <div v-if="isBatchConfirmOpen" class="dialog-overlay" @click.self="closeBatchConfirm">
      <div class="dialog-card dialog-confirm" role="dialog" aria-modal="true" aria-label="Batch delete interval rules">
        <div class="dialog-header">
          <h4>Batch Removal</h4>
        ...
        <div class="dialog-body">
          <p class="confirm-text">
            Are you sure you want to delete the selected interval freq limit rules?
          </p>
        </div>
        <div class="dialog-footer">
...
const loadRules = async () => {
  ...
  try {
    const data = await fetchIntervalRules(scope.value);
...
const createRule = async (payload) => {
  try {
    await createIntervalRule(scope.value, payload);
    await loadRules();
...
const updateRule = async (payload) => {
  try {
    await updateIntervalRule(scope.value, editingRuleId.value, payload);
    await loadRules();
...
const handleDeleteRule = async () => {
  try {
    await deleteIntervalRuleApi(scope.value, deleteTarget.value.id);
    await loadRules();
...
const handleBatchRemove = async () => {
  try {
    await deleteIntervalRules(scope.value, selectedRuleIds.value);
    await loadRules();
...
.chip {
  ...
  align-items: center;
  gap: 6px;
  border: 1px solid rgba(46, 158, 108, 0.35);
  background: var(--app-accent-soft);
  color: var(--app-accent);
  border-radius: 6px;
  padding: 4px 10px;
