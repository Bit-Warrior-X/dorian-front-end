...
const saveRule = () => {
  ...
  const payload = {
    ...
    countries: formState.value.countries.join(", "),
    behavior: formState.value.behavior,
    enabled: formState.value.enabled
  };
  if (editingRuleId.value) {
    const index = geoRules.value.findIndex((rule) => rule.id === editingRuleId.value);
    if (index !== -1) {
      geoRules.value[index] = { ...geoRules.value[index], ...payload };
    }
  } else {
    geoRules.value.unshift({
      id: `rule-${Date.now()}`,
      ...payload
    });
  }
...
const confirmDeleteRule = () => {
  ...
  geoRules.value = geoRules.value.filter((rule) => rule.id !== deleteTarget.value.id);
  deleteTarget.value = null;
...
const confirmBatchRemove = () => {
  if (!selectedRuleIds.value.length) return;
  geoRules.value = geoRules.value.filter((rule) => !selectedRuleIds.value.includes(rule.id));
